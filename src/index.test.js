...
function yulCompile(source) {
  try {
    ...
    const isErrors = (output.errors || []).filter((v) => v.type !== "Warning")
      .length;
    ...
    return {
      errors: null,
      bytecode:
        "0x" + output.contracts["Target.yul"].Target.evm.bytecode.object,
    };
  } catch (error) {
...
test("yulp should be yul", (t) => {
  ...
  t.equal(
    print(
      compile(
        ` object "SimpleStore" { code { enum Color(red, blue, green) return (Color.blue, Color.green) } } `
      ).results
    ),
    `object \"SimpleStore\" { code {  return (1, 2) } }`,
    "const"
  );

  t.equal(
    print(
      compile(` object "SimpleStore" { code { const nick := 0 } } `).results
    ),
  ...
  t.equal(
    print(
      compile(
        ` object "SimpleStore" { code { require(0, error"nick") require(1, error"john") } } `
      ).results
    ),
    `object \"SimpleStore\" { code {\nfunction require(arg, message) {\n  if lt(arg, 1) {\n    mstore(0, message)\n    revert(0, 32)\n  }\n}\n require(0, 0x01) require(1, 0x02) } }`,
    "const"
  );
});
